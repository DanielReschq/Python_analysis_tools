...
_style['marker'] = np.array(['o', 's', 'D', '^', 'v', '<', '>', 'p', 'h', 'o', 'o', 's', 'D', '^', 'v', '<', '>', 'p', 'h', 'o'])
_style['markerSize'] = np.array([6.5, 5.25, 5.75, 6.75, 6.75, 6.75, 6.75, 7.1, 6.9, 6.5,     6.5, 5.25, 5.75, 6.75, 6.75, 6.75, 6.75, 7.1, 6.9, 6.5])
_style['markerEdgeColor'] = np.array(['#b11823', '#dc6707', '#efc600', '#5dc661', '#60af92', '#505ea5', '#25305f', '#aa1966', '#231f20', '#000000',       '#b11823', '#dc6707', '#efc600', '#5dc661', '#60af92', '#505ea5', '#25305f', '#aa1966', '#231f20', '#000000'])
_style['markerFaceColor'] = np.array(['#cf9494', '#e6b594', '#f3e2a0', '#bbe0b5', '#b7d5c9', '#a8afce', '#8594bd', '#cd95b1', '#908e8f', '#666666',       '#cf9494', '#e6b594', '#f3e2a0', '#bbe0b5', '#b7d5c9', '#a8afce', '#8594bd', '#cd95b1', '#908e8f', '#666666'])
_style['Ldict'] = {}
...
def fit_residuals(params, sum, data, fitFunction):
    ...
    for L in list(data):
        if len(data[L][0]) == 2: # no errors
            ...
            res = np.concatenate((res, (y - fitFunction.func(x, L, params)) ))
...
def fit_leastsq(data, fitFunction, params=[]):
    ...
    params, dparams2, info, mesg, ierr = sp.optimize.leastsq(fit_residuals, params, args=(False, data, fitFunction), ftol=1.E-8, xtol=1.E-8, gtol=1.E-8, maxfev=10**6, full_output=True)

    if ierr < 5:
        redChi2 = fit_residuals(params, True, data, fitFunction)/(np.sum(np.fromiter((len(data[L]) for L in list(data)), np.double))-fitFunction.nparams())
...
def fit_minimize(data, fitFunction, params=[], **kwargs):
    ...
    result = sp.optimize.minimize(fit_residuals, params, args=(True, data, fitFunction), **kwargs)
...
def fit_precond(data, fitFunction, params=[]):
    ...
    result = sp.optimize.minimize(fit_residuals, params, args=(True, data, fitFunction), method='BFGS', tol=1.E-8, options={'gtol': 1E-8})
    params = result.x
        ...
        for name in list(var):
            if isinstance(var[name], np.ndarray):
                for i in range(len(var[name])):
                    output += '%s[%d] = %s\n'%(name, i, errText(var[name][i], dvar[name][i]))
            else:
                output += '%s = %s\n'%(name, errText(var[name], dvar[name]))
    else:
        output += 'Fit failed (%d): %s\n'%(ierr, mesg)

    if not silent: print("%s"%output)
...
def select(dataIn, LRange=[], gRange=[]):
    ...
    dataOut = {}
    for L in Llist:
        dataOut[L] = dataIn[L][np.where((dataIn[L][:,0] >= gRange[0]) & (dataIn[L][:,0] <= gRange[1]))]
...
def getDataRange(data, idx=0, margin=0.0):
    ...
    return np.array([ dataMin - margin*(dataMax-dataMin), dataMin + (1.+margin)*(dataMax-dataMin) ])
...
#-----------------------------------------------------------------------------------------
def rescaleAxis(dataIn, xfunc='x', yfunc='y', xIndex=0, yIndex=1, dxIndex=None, dyIndex=None, arg={}):
    ...
    for L in list(dataOut):
        for i in range(len(dataOut[L])):
            dict = arg
            dict['L'], dict['x'], dict['y'] = L, dataOut[L][i,xIndex], dataOut[L][i,yIndex]
...
def errText(val, err, maxDigit=16):
    ...
    for d in range(250):
        if int((err-int(err))*10.**d) != 0:
            errOrder = d
            break 
            
    return ('%.'+str(errOrder)+'f')%val + '(' + str(int(round(err*10**errOrder))) + ')'
...
def fitPoly(x, y, order, xerr=0, yerr=0):
    ...
    if type(xerr) == int and type(yerr) == int:
        ...
        err_func = lambda p, x, y, xerr, yerr: (y - poly(p,x)) / np.sqrt(xerr**2+yerr**2)
...
def intersect(p1, p2, x0=0., x1=1.): 
    ...
    if np.sign(f(x0, p1, p2)) == np.sign(f(x1, p1, p2)):
        ...
        x, r = sp.optimize.brentq(f, x0, x1,args=(p1, p2), xtol=1.e-15, rtol=1.e-15, full_output=True)
...
def bootstrapIntersect(data1, data2, order, nsamples=1000):
    ...
    return np.array([np.mean(resList[:,0]), np.sqrt(np.var(resList[:,0])), np.mean(resList[:,1]), np.sqrt(np.var(resList[:,1]))]), len(resList)
...
def crossingPoints(data, order=2, dLfunc='L+2', nsamples=1000):
    ...
    for i in range(len(Llist)):
        ...
        if np.isin(L1, Llist) & np.isin(L2, Llist):
            ...
            xpoints = np.vstack((xpoints, bootstrapIntersect(data[L1], data[L2], order, nsamples=nsamples)[0    ]))
...
def plotStyle(L):
    ...
    return {'color':          _style['markerEdgeColor'][styleIndex],
...
def errorbarStyle(L):
    ...
    return {'marker':          _style['marker'][styleIndex],
...
def setTicks(ax, xTicks=[], yTicks=[], minorxTicks=None, minoryTicks=None):
    ...
    if minorxTicks != None: ax.xaxis.set_minor_locator(ticker.AutoMinorLocator(minorxTicks))
    if minoryTicks != None: ax.yaxis.set_minor_locator(ticker.AutoMinorLocator(minoryTicks))
...
def legend(ax, plots, labels, legendLabelBreak=3, loc=(0.00, 0.02, 0.2), isInset=0):
    ...
    if nKeyColumns > 1:
        if len(loc) == 3:
            ...
            sys.exit('FS3.legend: please provide a 3-tuple for the location of the legend and the separation of the key columns.')
    else:
        legendLoc.append([loc[0], loc[1]])
    
    if isInset == 0:
        for i in range(nKeyColumns):
            plt.gca().add_artist(ax.legend(plots[i*legendLabelBreak:(i+1)*legendLabelBreak], labels[i*legendLabelBreak:(i+1)*legendLabelBreak], loc=legendLoc[i], frameon=False, labelspacing=0.4, handletextpad=-0.25, prop={'size': mpl.rcParams['legend.fontsize']}))
    else:
        for i in range(nKeyColumns):
            plt.gca().add_artist(ax.legend(plots[i*legendLabelBreak:(i+1)*legendLabelBreak], labels[i*legendLabelBreak:(i+1)*legendLabelBreak], loc=legendLoc[i], frameon=False, labelspacing=0.4, handletextpad=-0.25, prop={'size': int(mpl.rcParams['legend.fontsize']*0.8)}))
...
def addInset(fig, width=0.35, height=0.35, loc=0, xlabel='', ylabel=''):
    ...
    if loc == 0:
        ...
        ax.tick_params(labelleft=False, labelright=True, labeltop=False, labelbottom=True, labelsize=int(mpl.rcParams['xtick.labelsize']*0.8))
        ax.xaxis.labelpad = -2
    elif loc == 1:
        ax = fig.add_axes([0.14, 0.14, width, height])
        ax.xaxis.set_label_position("top")
        ax.yaxis.set_label_position("right")
        ax.tick_params(labelleft=False, labelright=True, labeltop=True, labelbottom=False, labelsize=int(mpl.rcParams['xtick.labelsize']*0.8))
    elif loc == 2:
        ax = fig.add_axes([0.89-width, 0.14, width, height])
        ax.xaxis.set_label_position("top")
        ax.yaxis.set_label_position("left")
        ax.tick_params(labelleft=True, labelright=False, labeltop=True, labelbottom=False, labelsize=int(mpl.rcParams['xtick.labelsize']*0.8))
    elif loc == 3:
        ax = fig.add_axes([0.89-width, 0.865-height, width, height])
        ax.xaxis.set_label_position("bottom")
        ax.yaxis.set_label_position("left")
        ax.tick_params(labelleft=True, labelright=False, labeltop=False, labelbottom=True, labelsize=int(mpl.rcParams['xtick.labelsize']*0.8))
        ax.xaxis.labelpad = -2
    else:
        sys.exit('FS3.addInset: valid values for loc are: 0 (top left), 1 (bottom left), 2 (bottom right), 3 (top right).')
...
if (__name__ == '__main__'):
    print('This is the Finite Size Scaling Suite (FS3) module, version %s'%__version__)
